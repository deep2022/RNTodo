...
const Settings = ({navigation}) => {
  ...
  const obj = {
    token: userDetails.Auth.token,
    entityId: userDetails.Auth.id,
    entityType: userDetails.Auth.user,
  };
  const {dark, setDark, lang, setLang} = useContext(Mode);
  const [isEnabled, setIsEnabled] = useState(false);
  const toggleSwitch = () => (
    setDark(dark == 'dark' ? 'light' : 'dark'),
    setIsEnabled(dark === 'dark' ? true : false)
  );
  useEffect(() => {
    I18n.locale = lang;
  ...
  return (
    <View
      style={
        dark === 'light' ? {flex: 1} : {flex: 1, backgroundColor: 'black'}
      }>
      <View
        style={{
          flexDirection: 'row',
      ...
      <View
        ...
        <Picker
          style={
            dark === 'dark' ? {width: '34%', color: 'white'} : {width: '34%'}
          }
          selectedValue={lang}
      ...
      <Button
        style={{backgroundColor: 'blue', alignSelf: 'center', bottom: '-55%'}}
        color={'white'}
        onPress={() => (removeToken(), dispatch({type: 'LOGOUT'}))}>
        Log Out
      </Button>
    </View>
  );
